...
#TODO 
split this chunk so I can turn off the progress display  
...
Calculating the mean across a 3X3 matrix and storing that value in the middle pixel of that matrix.  The resolution of the imagery comes into play here - RAP has 30m resolution so a 3X3 matrix represents a 90mX90m area on the ground.  Size and mobility/territoriality of a target organism might influence the choice of matrix size as well.
```{r}
#| label: focal mean
focalities <- list()
for (n in layerList) {
